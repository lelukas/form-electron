...
export type OnProgress = (update: {
	...
	message?: string
	entry?: { nome: string; status: string; error?: string }
}) => void
...
async function selectInSelectBlock(
	eJS: ExecuteJS,
	label: string,
	value: string,
	...
	while (Date.now() - start < 8000) {
		...
		if (ok) {
			onProgress({ type: "log", message: `✅ Select "${label}" → ${value}` })
			return
		}
		await sleep(300)
	}
...
async function selectInCheckboxBlock(
	eJS: ExecuteJS,
	label: string,
	value: string,
	onProgress: OnProgress,
...
async function clickRadioByText(
	eJS: ExecuteJS,
	text: string,
	onProgress: OnProgress,
): Promise<void> {
...
async function clearChecked(eJS: ExecuteJS, onProgress: OnProgress): Promise<void> {
	await eJS(
		`document.querySelectorAll('div[aria-checked="true"]').forEach(function(el){el.click()})`,
	)
...
function createSetInputFiles(wc: any): SetInputFiles {
	return async (_selector: string, files: string[]) => {
		...
		await wc.executeJavaScript(
			`document.querySelector('div[aria-label="Adicionar arquivo"]')?.click()`,
		)
