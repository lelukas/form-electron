...
export function createForm<T extends Record<string, any>>(initial: T, rules: Rules<T>) {
  ...
  let showErrors = $derived(submitted || Object.keys(touched).length > 0)

  function field(key: keyof T) {
    return {
      get value() { return values[key] },
      set value(v: T[keyof T]) { values[key] = v },
      get error() { return showErrors ? errors[key] : undefined },
      touch() { touched[key] = true },
    }
  }
